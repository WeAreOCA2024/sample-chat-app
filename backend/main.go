...
func main() {
	...
	r.GET("/get/friend/profile/:name", func(c *gin.Context) {
		name := c.Param("name")
		myId := getUserByName(db, name)[0].ID
		friendProfile := getFriendProfileById(db, myId)
...
func getFriendProfileById(db *sql.DB, id int) []Profile {
	rows, err := db.Query("SELECT * FROM profile WHERE id IN (SELECT user1_pid FROM friends where user2_id=$1) or id IN (SELECT user2_pid FROM friends where user1_id=$1)", id)
	if err != nil {
		return nil
	}
	defer rows.Close()
